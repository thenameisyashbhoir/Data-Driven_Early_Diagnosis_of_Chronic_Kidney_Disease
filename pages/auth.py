...
def render_login():
    ...
    # Remove default streamlit top padding + hide deploy button
    st.markdown("""
    <style>
    .block-container { padding-top: 1rem !important; }
    .stDeployButton, [data-testid="stDeployButton"],
    button[kind="header"], [data-testid="stToolbar"] { display:none!important; }
    </style>
    ...
    with tab_login:
        ...
        with col_form:
            ...
            # Chrome autocomplete / password save support
            st.markdown("""
            <script>
            (function() {
              function addAutocomplete() {
                var inputs = window.parent.document.querySelectorAll(
                  'input[data-testid="stTextInput"], input[type="text"], input[type="password"]'
                );
              ...
              addAutocomplete();
              setTimeout(addAutocomplete, 600);
              setTimeout(addAutocomplete, 1500);
            })();
            </script>
            """, unsafe_allow_html=True)
